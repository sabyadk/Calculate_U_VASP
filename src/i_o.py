    ...
    for line in f:
      try:
       ...
       if line.split('=')[0]=='fr':
         ...
         fr=int(line.split('=')[1])
       if line.split('=')[0]=='isim':

         isim=int(line.split('=')[1])


       if line.split('=')[0]=='hour':

         h=int(line.split('=')[1])
       if line.split('=')[0]=='minute':

         m=int(line.split('=')[1])
       if line.split('=')[0]=='second':

         s=int(line.split('=')[1])
       if line.split('=')[0]=='path':

         path=(np.array(line.split(',')[1:-1]).astype(str))[0]
         print(np.shape(path))

       if line.split('=')[0]=='path2':

         path2=(np.array(line.split(',')[1:-1]).astype(str))[0]
         print(np.shape(path))
       ...
       if line.split('=')[0]=='V.LSORBIT':

          LSORBIT=int(line.split('=')[1])
       ...
       if line.split('=')[0]=='V.NSW':

         NSW=int(line.split('=')[1])
       if line.split('=')[0]=='V.ISIF':

         ISIF=int(line.split('=')[1])
       if line.split('=')[0]=='V.IBRION':

         IBRION=int(line.split('=')[1])
       if line.split('=')[0]=='V.EDIFFG':
