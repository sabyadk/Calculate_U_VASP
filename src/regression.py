...
def regress(alpha,mat,font):
    ...
    for i in range(len(alpha)):
      ...
      for j in range(N):
        ...
        n_bare[i,j,:]=np.array(charge_read('./'+str(-1*alpha[i])+'/'+str(j)+'/bare/OUTCAR',len(mat),2))[Nt-N:Nt] 
        n_scf[i,j,:]=np.array(charge_read('./'+str(-1*alpha[i])+'/'+str(j)+'/scf/OUTCAR',len(mat),2))[Nt-N:Ns+Nt]
        print(np.array(charge_read('./'+str(-1*alpha[i])+'/'+str(j)+'/scf/OUTCAR',len(mat),2))[Nt-N:Nt])      
      ...
      for j in range(N):
         ...
         if(diag==1):
            if(i==j):      
                ...
                l3,= plt.plot(alpha,Y,color='b')
                l4,= plt.plot(alpha,T,color='r')
     plt.legend([l1,l2,l3,l4],['bare response','SCF response','scf fit','bare fit'],fontsize=font,loc=3) 
     plt.axis('tight')
